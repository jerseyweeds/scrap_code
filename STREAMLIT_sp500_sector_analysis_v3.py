...
denominator_options = df.columns.difference(excluded_columns)
...
numerator = st.sidebar.selectbox('Select Numerator', numerator_options, index=numerator_options.get_loc(default_numerator))
denominator = st.sidebar.selectbox('Select Denominator', denominator_options, index=denominator_options.get_loc(default_denominator))
...
if select_all_sectors:
    selected_sectors = unique_sorted_sectors
else:
    selected_sectors = st.sidebar.multiselect('Select Sectors', unique_sorted_sectors, default=unique_sorted_sectors)
...
for sector in selected_sectors:
    sector_companies = df[df['Sector'] == sector]['Company'].unique()
...
def update_charts():
    ...
    if not filtered_data.empty:
        ...
        st.altair_chart(alt.Chart(chart_data).mark_line().encode(
            ...
            y=alt.Y('Ratio', title=f'{numerator} / {denominator}'),
            color=alt.Color('Sector:N', title='Sector'),
            tooltip=['Date:T', 'Ratio', 'Sector:N']
        ).properties(
            width=800,
            height=400,
